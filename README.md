...
1. Make sure the following thing are being tracked in that local repo (and
    ...
    * `src/`
    * `config/`
    * `README.md` (this file, copy from the sample repo)
    * `build.gradle`
    * `instructor-tests.gradle` (test url updated for lab 4)
...
1. To get these files into your lab3 repo, you can either:
    * In a fresh folder `git clone https://gitlab.engr.ship.edu/chuo/swe200-lab4-sample`, drag and drop the files to your lab3 folder
    * Or, create the files yourself, then copy and paste the text
...
* The initial lab 3 push to the server must happen after every member has
  concluded his/her lab 3.
* As said in the syllabus, for team projects, if the submission is late for a
  day, every member will lost a late day, and so on. If a member is running out
  of late days, other members may use extra days to cover this member.
